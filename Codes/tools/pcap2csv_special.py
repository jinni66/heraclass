...
def parse_label(pcap_file):
    ...
    Example: "com.zhejiangdaily.pcap" -> "com.zhejiangdaily"
    """
    filename = os.path.basename(pcap_file)
    label = os.path.splitext(filename)[0]  # Remove the .pcap extension
    return label
...
def process_directory(input_dir, output_dir):
    ...
    print(f"Found {len(pcap_files)} PCAP files, starting parsing...")
...
def main():
    ...
    if args.pcap_file:
        ...
        print("Please specify either --pcap-file or --input-dir to process PCAP files.")
